#
###########################################################################################
##### Discovery analysis [case vs control] (WCM PNET dataset {insulinomas only} vs. Fadista control pancreatic islets)
###########################################################################################
...
metadata_WCM=read.csv('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/_WCM_PNET_RNAseq_data/PNET_metadata_indicates_whether_PNET_is_insulionoma_or_not.csv')
...
colnames(metadata_WCM) = c('files','names','Dx','Loc_or_Met','Sex','Age','Insulinoma')

# keep only insulinomas
metadata_WCM=metadata_WCM[which(metadata_WCM$Insulinoma == 'Yes'),]
metadata_WCM=metadata_WCM[,-ncol(metadata_WCM)]
...
metadata=rbind(metadata_WCM,metadata_Fadista)
...
# Do you wish to use the default directory: '/home/nai2008/.cache/BiocFileCache'?
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_SV_plots.pdf')
...
design(dds) = ~ SV_1 + SV_2 + SV_3 + SV_4 + SV_5 + SV_6 + SV_7 + SV_8 + SV_9 + SV_10 + SV_11 + SV_12 + SV_13 + SV_14 + SV_15 + SV_16 + SV_17 + Dx
...
# 41 rows did not converge in beta
...
summary(rr)
# out of 31815 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 8470, 27%
# LFC < 0 (down)     : 6718, 21%
# outliers [1]       : 0, 0%
# low counts [2]     : 8019, 25%
# (mean count < 0)
# [1] see 'cooksCutoff' argument of ?results
# [2] see 'independentFiltering' argument of ?results

pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_Cooks_distances.pdf')
...
# save releveant data
save(se, gse, tpm, ddsClean, rr, vsd, file='/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/WCM_insulinomasONLY_and_Fadista_datasets_DESeq2_DEbyDx_BUNDLE.rda')
# load('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/WCM_insulinomasONLY_and_Fadista_datasets_DESeq2_DEbyDx_BUNDLE.rda') # se, gse, tpm, dds, ddsClean, rr, vsd

################ Downstream analysis

source('/athena/masonlab/scratch/users/nai2008/ivanov_functions.R')
library(DESeq2)
load('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/WCM_insulinomasONLY_and_Fadista_datasets_DESeq2_DEbyDx_BUNDLE.rda') # se, gse, tpm, dds, ddsClean, rr, vsd, rld
...
colData(ddsClean)$Dx=as.vector(colData(ddsClean)$Dx)
colData(ddsClean)$Dx[which(colData(ddsClean)$Dx=='PNET')] = 'Insulinoma'
colData(ddsClean)$Dx=factor(colData(ddsClean)$Dx, levels=c('Control','Insulinoma'))
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_M1_and_M2_heatmaps.pdf')
...
length(which(out$TF==TRUE)) # 1893

save(out, file='/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/WCM_insulinomasONLY_and_Fadista_datasets_DE_genes_byDx.rda')
# load('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/WCM_insulinomasONLY_and_Fadista_datasets_DE_genes_byDx.rda')

# print table of DE genes
write.csv(out,file="/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/data_tables/WCM_insulinomasONLY_and_Fadista_datasets_DE_genes_byDx.csv", row.names=FALSE)

### calculate concordance scores with the mouse transcriptome for M1 and M2 genes

load('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/M1_and_M2_gene_sets.rda') #M1_marker_panel,M2_marker_panel,genes_upregulated_in_M1_and_downregulated_in_M2,genes_upregulated_in_M2_and_downregulated_in_M1

pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_M1_and_M2_concordance_scores.pdf')
...
length(which(disco.score>0))/length(disco.score) # 0.5853659
...
length(which(disco.score>0))/length(disco.score) # 0.3636364
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_goseq_pwf_plot.pdf')
plotPWF(pwf)
dev.off()
...
length(which(GO.KEGG.wall$over_represented_FDR<=0.1)) #16
...
write.csv(GO.KEGG.wall_sig, file = '/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/data_tables/WCM_insulinomasONLY_and_Fadista_datasets_DEbyDx_ORA.csv')
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_ORA_plot.pdf')
par(mar=c(5.1, 9.5, 4.1, 0))
barplot(ora_to_plot$neg_log_over_represented_FDR, names.arg= ora_to_plot$term, xlim=c(0,2+round(max(ora_to_plot$neg_log_over_represented_FDR))), horiz=TRUE, col='black', xlab = '-Log(FDR)', ylab=NA, las=1, cex.names=0.7)
dev.off()
...
save(genes_of_interest_rr_subset, file = '/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/WCM_insulinomasONLY_and_Fadista_datasets_genes_of_interest_rr_subset.rda')
# load('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/rdas/WCM_insulinomasONLY_and_Fadista_datasets_genes_of_interest_rr_subset.rda') # genes_of_interest_rr_subset
...
mycol[which(mycol=="Insulinoma")]='red'

pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_genes_of_interest_TPMs.pdf')
...
for (i in 1: length(genes_to_plot$ensembl)){
	...
	x=factor(as.vector(ddsClean$Dx),levels=c('Control','Insulinoma'))
...
# labs=c('CDH1','CXCL2','IL1A','IL6','PPARD','NR2F6','NFKBIZ','TNFAIP3','CCL22','TNF','CLEC7A','ITGAX','ARG1','CLEC10A','CSF2','IL23A','IL12B')
# col=paste(paste0("'",genes.to.show$col[match(labs,genes.to.show$gene_symbol)],"'"),sep="",collapse=",")

library(EnhancedVolcano)

pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_volcano_plot_DEbyDx.pdf')
...
EnhancedVolcano(toptable = as.data.frame(rr[,c(2,6)]),
	...
	pCutoff = 0.1,
	FCcutoff=1,
	title='Insulinomas vs. non-neoplastic pancreatic islets',
...
pnet_tpm_mat=tpm[,which(as.vector(ddsClean$Dx)=='Insulinoma')]
...
pnet_tpm_dup_rm = tpm_dup_rm[,which(as.vector(ddsClean$Dx)=='Insulinoma')]
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_inSilico_deconvolution.pdf')
...
res_quantiseq_pnet %>%
  ...
  ggplot(aes(x=sample, y=fraction, fill=cell_type)) +
    ...
    ggtitle('Immune cell type deconvolution (quanTIseq); Insulinoma Samples')
...
res_epic_pnet %>%
  ...
  ggplot(aes(x=sample, y=fraction, fill=cell_type)) +
    ...
    xlab('Sample') +
    guides(fill=guide_legend(title="Cell Types")) +
    ggtitle('Immune cell type deconvolution (EPIC); Insulinoma Samples')
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_deconvolution_cellType_comparisons_quantiseq.pdf')
...
for (i in 1:nrow(res_quantiseq_pnet_df)){
	...
	dx=c(rep('Insulinoma',times=ncol(res_quantiseq_pnet_df)-1),rep('Control',times=ncol(res_quantiseq_control_df)-1))
	dx=factor(dx,levels=c('Control','Insulinoma'))
	...
	zag2=paste0('{ Control median = ', signif(median_control,2), '; Insulinoma median = ', signif(median_PNET,2),' }')
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_deconvolution_cellType_comparisons_epic.pdf')
...
for (i in 1:nrow(res_epic_pnet_df)){
	...
	dx=c(rep('Insulinoma',times=ncol(res_epic_pnet_df)-1),rep('Control',times=ncol(res_epic_control_df)-1))
	dx=factor(dx,levels=c('Control','Insulinoma'))
...
for (i in 1:nrow(abundance_matrix)){
	p=wilcox.test(as.numeric(abundance_matrix[i,which(colnames(abundance_matrix) == 'Insulinoma')]), as.numeric(abundance_matrix[i,which(colnames(abundance_matrix) == 'Control')]))
	p.wilcox.test[i]=p$p.value
}

FDR=p.adjust(p.wilcox.test,method='BH')

pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_deconvolution_cellType_comparisons_cibersort.pdf')

for (i in 1:nrow(abundance_matrix)){

	cell_type_data=as.numeric(abundance_matrix[i,])

	dx=factor(colnames(abundance_matrix),levels=c('Control','Insulinoma'))

	median_PNET=median(cell_type_data[which(dx=='Insulinoma')])
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_deconvolution_cellType_comparisons_mcpCounter.pdf')
...
pdf('/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/RNAseq_analysis/pdfs/WCM_insulinomasONLY_and_Fadista_datasets_deconvolution_cellType_comparisons_xCell.pdf')
